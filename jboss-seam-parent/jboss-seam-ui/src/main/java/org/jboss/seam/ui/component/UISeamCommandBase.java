...
public abstract class UISeamCommandBase extends UIOutput implements
		ActionSource2 {
	...
	public String getUrl() throws UnsupportedEncodingException {
		...
		ViewUrlBuilder url = new ViewUrlBuilder(viewId, getFragment(),
				!isPortletRequest(getFacesContext()));
		...
		if (viewId != null && isIncludePageParams()) {
			Map<String, Object> pageParameters = Pages.instance()
					.getStringValuesFromModel(context, viewId, usedParameters);
		...
		PropagationType propagationType = PropagationType
				.valueOf(getPropagation());
		if (propagationType == PropagationType.DEFAULT
		...
		{
			UIConversationPropagation uiPropagation = UIConversationPropagation
					.newInstance();
		...
		if (taskInstanceValueExpression != null) {
			...
			uiTaskId.setValueExpression("taskInstance",
					taskInstanceValueExpression);
	...
	public UISelection getSelection() {
		...
		if (parentUIData != null) {
			if (parentUIData.getValue() instanceof DataModel) {
				String dataModelExpression = parentUIData.getValueExpression(
	...
	public void removeActionListener(ActionListener listener) {
		...
		// throw new
		// UnsupportedOperationException("Action listeners not supported by s:link/s:button");
	}
	...
	public javax.faces.el.MethodBinding getAction() {
		return new org.jboss.seam.ui.util.cdk.MethodExpressionToMethodBinding(
				getActionExpression());
	}

	private static boolean isPortletRequest(FacesContext facesContext) {
		return PORTLET_REQUEST != null
				&& PORTLET_REQUEST.isInstance(facesContext.getExternalContext()
						.getRequest());
	}
